...
class OCTree:
	...
	def from_vertices(self, vertices, vert_ind):
		self.start = np.min(vertices[vert_ind], axis = 0) - 1e-8
		self.end = np.max(vertices[vert_ind], axis = 0) + 1e-8
	...
	def from_triangles(self, vertices, faces, face_ind):
		self.start = np.min(vertices[faces[face_ind]].min(axis = 1), axis = 0) - 1e-6 
		self.end = np.max(vertices[faces[face_ind]].max(axis = 1), axis = 0) + 1e-6 
		grid_size = (self.end - self.start) / default_grid_num
		grid_num = default_grid_num ** 3
